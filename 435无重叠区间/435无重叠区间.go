		...
		if intervals[i][0] < temp[1] {
			if intervals[i][1] > temp[1] {
				if i < n-1 && intervals[i+1][0] >= intervals[i][1] {
					range1 := temp[1] - temp[0]
					range2 := intervals[i][1] - intervals[i][0]
					if range1 > range2 {
						temp = intervals[i]
					}
				}
...
func eraseOverlapIntervalsOfficial(intervals [][]int) int {
	n := len(intervals)
	if n < 2 {
		return 0
	}
...
func eraseOverlapIntervalsOfficial1(intervals [][]int) int {
	n := len(intervals)
	if n < 2 {
		return 0
	}
